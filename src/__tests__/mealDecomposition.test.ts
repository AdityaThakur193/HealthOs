...
describe("enrichMealAnalysisWithIFCT Ingredient Decomposition Pipeline", () => {
  ...
  it("should accurately enrich complex restaurant meal: Butter Chicken, Butter Naan, and Chilli Chicken with supplemental standards", () => {
    ...
    expect(butterNaan.estimatedCalories).toBe(424);
    expect(butterNaan.proteinG).toBe(9.5);
    expect(butterNaan.fatG).toBe(12.9);
    expect(butterNaan.carbsG).toBe(66.9);

    // 3. Chilli Chicken
    expect(chilliChicken.unmatched).toBe(false);
    expect(chilliChicken.partialMatch).toBe(false);
    expect(chilliChicken.estimatedCalories).toBe(536);
    expect(chilliChicken.proteinG).toBe(36.4);
    expect(chilliChicken.fatG).toBe(33.9);
    expect(chilliChicken.carbsG).toBe(20.7);

    // Overall meal totals
    expect(enriched.totalCalories).toBe(589 + 424 + 536); // 1549 kcal
    expect(enriched.totalProteinG).toBe(Math.round((37.2 + 9.5 + 36.4) * 10) / 10); // 83.1g
    expect(enriched.totalFatG).toBe(Math.round((45.8 + 12.9 + 33.9) * 10) / 10); // 92.6g
  });
});
